  ...
  public final void setMaxSize(final int amount) {
    ...
    synchronized (this.m_renderer) {
      ...
      synchronized (this) {
        ...
        this.minXSearch();
        if (this.m_minX != xmin) {
          this.firePropertyChange(ITrace2D.PROPERTY_MIN_X, xmin, this.m_minX);
        }

        final double xmax = this.m_maxX;
        this.maxXSearch();
        if (this.m_maxX != xmax) {
          this.firePropertyChange(ITrace2D.PROPERTY_MAX_X, xmax, this.m_maxX);
        }

        final double ymax = this.m_maxY;
        this.maxYSearch();
        if (this.m_maxY != ymax) {
          this.firePropertyChange(ITrace2D.PROPERTY_MAX_Y, ymax, this.m_maxY);
        }

        final double ymin = this.m_minY;
        this.minYSearch();
        if (this.m_minY != ymin) {
          this.firePropertyChange(ITrace2D.PROPERTY_MIN_Y, ymin, this.m_minY);
        }
      }
    }
  }
